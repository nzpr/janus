...
/// Convert a token string into a protected static `Authorization` header value.
pub(crate) fn protect_bearer_auth_header(token: &str) -> Result<&'static str> {
    ...
    if token_bytes.len() > buf.len() - AUTH_HEADER_PREFIX.len() {
        buf.zeroize();
        return Err(anyhow!(
    ...
    let total = AUTH_HEADER_PREFIX.len() + token_bytes.len();
    buf[AUTH_HEADER_PREFIX.len()..total].copy_from_slice(token_bytes);
    ...
    let header_str = std::str::from_utf8(&buf[..total])
        .context("reading Authorization header as UTF-8")?;
    HeaderValue::from_str(header_str).context("validating Authorization header")?;
    ...
    F: FnMut(&mut [u8]) -> std::io::Result<usize>,
{
    ...
    if total == AUTH_HEADER_PREFIX.len() {
        buf.zeroize();
        return Err(anyhow!(
            "API key must be provided via stdin (e.g. printenv OPENAI_API_KEY | janus)"
        ));
    }

    let key_bytes = &buf[AUTH_HEADER_PREFIX.len()..total];
    if let Err(err) = validate_auth_header_bytes(key_bytes) {
        buf.zeroize();
        return Err(err);
    }
...
mod tests {
    ...
    #[test]
    fn protects_bearer_header_from_direct_token() {
        let header = protect_bearer_auth_header("sk-abc123").expect("protected header");
        assert_eq!(header, "Bearer sk-abc123");
    }
